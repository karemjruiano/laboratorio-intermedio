...
nombre_excel = 'datos_difraccion_electrones.xlsx'
with pd.ExcelWriter(nombre_excel, engine= "xlsxwriter") as writer:
    data_broglie.to_excel(writer, sheet_name='Hoja1', index=False)
    data_bragg.to_excel(writer, sheet_name='Hoja2', index=False)
    data_dist.to_excel(writer, sheet_name='Hoja3', index=False)
    data_h.to_excel(writer, sheet_name='Hoja4', index=False)
